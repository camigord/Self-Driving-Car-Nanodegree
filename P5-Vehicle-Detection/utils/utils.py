...
def extract_features(imgs, color_space=None, spatial_size=(32, 32),
...
def single_img_features(img, color_space=None, spatial_size=(32, 32),
    ...
    if color_space is not None:
        feature_image = convert_color(img, conv='BGR2'+ color_space)
...
def search_windows(img, windows, clf, scaler, pca, color_space=None,
    ...
    '''
    Define a function to search for cars in an image given the specified search windows
    '''
...
def find_cars(img, ystart, ystop, scale, svc, X_scaler, pca, orient, pix_per_cell, cell_per_block, hist_bins, color_space, cells_per_step, get_all = False):
    ...
    for xb in range(nxsteps):
        for yb in range(nysteps):
            ...
            test_prediction = svc.predict(reduced_features)

            if test_prediction == 1 or get_all == True:
...
# Define a single function that can extract features using hog sub-sampling and make predictions
def find_cars2(img, xstart, xstop, ystart, ystop, scale, svc, X_scaler, pca, orient, pix_per_cell, cell_per_block, hist_bins, color_space, cells_per_step, get_all = False):
